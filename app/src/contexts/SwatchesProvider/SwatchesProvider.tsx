...
import { DEFAULT_NUM_RESULTS, MAX_NUMBER_SWATCHES } from "constants/defaultValues";
...
const SwatchesContext = createContext<SwatchesContextType>({} as SwatchesContextType);
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case ACTIONS.REMOVE_SWATCH:
      return {
        ...state,
        selectedCards: state.selectedCards.filter(swatch => swatch.id !== action.payload.id),
        selectedIdList: state.selectedIdList.filter(id => id !== action.payload.id)
      };
...
export default function SwatchesProvider({ children }: SwatchesContextProps) {
  const [{ numResults, selectedCards, selectedIdList }, dispatch] = useReducer(reducer, initialState);
  ...
  return (
    <SwatchesContext.Provider value={{
      numResults,
      selectedCards,
      selectedIdList,
